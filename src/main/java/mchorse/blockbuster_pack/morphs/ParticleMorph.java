        ...
        if (this.frequency != 0 && this.tick >= this.delay && this.tick % this.frequency == 0 && alive)
        {
            ...
            if (vanillaCap || morphCap)
            {
                if (!target.world.isRemote && this.mode == ParticleMode.VANILLA && this.vanillaType != null)
                {
                    ...
                    ((WorldServer) target.world).spawnParticle(this.vanillaType, true, x, y, z, this.count, this.vanillaDX, this.vanillaDY, this.vanillaDZ, this.speed, this.arguments);
                }
                else if (target.world.isRemote && this.mode == ParticleMode.MORPH && this.morph != null && this.morphParticles.size() < this.maximum)
                {
