...
public class ModelExporter
{
    ...
    public ModelExporter(EntityLivingBase entity, RendererLivingEntity render)
    {
        this.entity = entity;
        this.render = render;
    }
    ...
    {
        Field[] fields = RendererLivingEntity.class.getDeclaredFields();
        Field field = null;

        for (Field target : fields)
        {
            if (target.getType().equals(ModelBase.class))
            {
                field = target;
        ...
        if (field != null)
        {
            try
            {
                ...
                return (ModelBase) field.get(this.render);
            }
            catch (Exception e)
            {
                e.printStackTrace();
    ...
    {
        TextureOffset offset = model.getTextureOffset(box.field_78247_g);
